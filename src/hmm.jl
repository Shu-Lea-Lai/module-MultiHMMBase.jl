...
HMM(a::AbstractVector{T}, A::AbstractMatrix{T}, B::AbstractMatrix{<:Distribution{F}}) where {F,T} = HMM{F,T}(a, A, B)
HMM(A::AbstractMatrix{T}, B::AbstractMatrix{<:Distribution{F}}) where {F,T} = HMM{F,T}(ones(size(A)[1])/size(A)[1], A, B)
...
function assert_hmm(a::AbstractVector,
    ...
    @argcheck length(a) == size(A,1) == size(B,2) #the second dimension of B is how many states there are
    return true
end
...
function rand(rng::AbstractRNG, hmm::AbstractHMM{Univariate}, z::AbstractVector{<:Integer})
    M,T = size(hmm.B, 1),length(z)
    y = Matrix{Float64}(undef, M, T)
    for t in OneTo(T), m in OneTo(M)
        y[m,t] = rand(rng, hmm.B[m,z[t]])
    end
    y
end
...
```
"""
size(hmm::AbstractHMM, dim=:) = (size(hmm.B,2), length(hmm.B[1, 1]))[dim]
"""
